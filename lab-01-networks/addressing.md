...
### Why IPv6 is disabled on the internal segments

`ipv6.method disabled` on the three internal profiles. In a segmentation lab, active IPv6 introduces a parallel addressing plane (link-local, SLAAC autoconfiguration) that can establish connectivity along paths the IPv4 design never accounted for. That is exactly the kind of undocumented route that invalidates an isolation test.

**The NAT leg is the exception, and it was not deliberate.** No custom profile was created for `enp0s3`; it runs on the cloned `Wired connection 1` with default settings, and VirtualBox's NAT advertises an IPv6 prefix. The interface therefore holds three IPv6 addresses — two global via SLAAC, one link-local — while the internal legs hold only link-local.

Verified in `configs/lab-router-network.txt`:

    inet6 fd17:625c:f037:2:8ac:6603:7f33:1804/64 scope global temporary dynamic
    inet6 fd17:625c:f037:2:a00:27ff:fe07:79b5/64 scope global dynamic mngtmpaddr
    inet6 fe80::a00:27ff:fe07:79b5/64 scope link

This matters for Session 2: filtering rules written for IPv4 alone do not constrain an active IPv6 plane. The external leg must either be brought under the same policy or have IPv6 explicitly disabled before inter-segment isolation can be claimed as complete.
...
Baseline for Session 2: replies from `10.10.10.1` arrive with `ttl=64`, meaning zero routing hops — the reply originates from a local interface of the responding host. Once forwarding is enabled, traffic crossing the router toward an external destination should arrive with `ttl=63`. That decrement of one is the direct evidence of a routing hop; its absence would mean the packet never crossed.
